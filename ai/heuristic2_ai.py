...
def getPlaceAction(state, weights, actions):
    weight_place = weights.get("weight_place")
    weight_defense = weights.get("weight_defense")
    weight_spread = weights.get("weight_spread")
    ...
    for a in actions:
        ...
        cur_territory = state.board.territories[territory_id]

        for neighbor in cur_territory.neighbors:
            if state.owners[neighbor] != state.current_player:
                opponent_count += 1

        defense_score = opponent_count * weight_defense
        spread_score = (1 / (1 + len(cur_territory.neighbors))) * weight_spread
        total_score = defense_score + spread_score

        if total_score > weight_place * max_opponent_count:
            best_action = a
            max_opponent_count = total_score
